...
def get_data_from_api():
    ...
    all_surveys_with_resources =  pd.json_normalize(all_surveys_with_resources, max_level=0)
...
def export_data_to_csv(api_data):
    ...
    all_filenames = ["datalib_all_info", "datalib_users"]
    folder = "output"
    today = str(date.today()).replace("-", "_")

    for df, filename in zip(api_data, all_filenames):
...
def process_data(data):
    raw_surveys_with_resources, users = api_data
    processed_surveys_with_resources = raw_surveys_with_resources[['assessment_status', 'collection_method',
...
def main():
    ...
    # Get users and surveys information
